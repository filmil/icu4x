...
This should result in Crate names always being unambiguously mappable to/from
...
consider stable: * Where another crate may need access to non-public API * Where
we want to expose some non-public API for testing or benchmarking * Where we
want to expose some non-public API for helper macros
...
Since the `date_style` and `time_style` fields can accept only valid values, and
...
Implementing comparison traits can be problematic and should only be done for
...
In the case of `Option<char>`, `None` is
...
Even a simple if/else block can be expressed more idiomatically using a match
...
While it's still an open question in the Rust community as to what the best way
...
If data access is expected to fail occasionally (e.g. looking up properties in a
...
If this check does not occur immediately before the data access (i.e. shortly
...
When creating functions which can fail to return a value: * Use **IcuResult**
for all errors, or any cases where a user facing message is needed. * Use
**Option** for data accessors where "no data available" is a valid response
(i.e. it's not an error per se). * Especially in cases where we expect the
caller to have a reasonable response to getting
[None](https://doc.rust-lang.org/std/option/enum.Option.html#variant.None). *
Use a different enum for non-error cases with multiple return types (which can't
use `Option`).

Examples: * Does file with this path exists? - Option. * Is there an element
with this key in the list? - Option * Try to open a file - Result * Try to parse
a string into a valid Language Identifier - Result
...
In cases where a hard limit on input is not possible, you can use methods such
as
...
Most ICU4X code will not work in
...
Thus we could provide one or more ICU4X traits bound to things like `str` to
